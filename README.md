...
- **Service-level objectives, including the case where the objective can't be measured.** I defined the fleet's availability and latency SLOs as multi-window burn-rate alerts: a **99% availability** budget where a fast burn (**14.4×** the budget, on the 1h *and* 5m windows) pages and a slow burn (**6×**, on 6h *and* 30m) opens a ticket, plus a latency objective of **95% of inbound requests under 250ms**. Each is gated on a minimum sample count derived from the standard error at the threshold band, not a round number. The rule I'd point to fires when a service is *unmeasurable* — restricting the denominator honestly meant the two highest-traffic services produced no series rather than a zero, so I made that silence alert: a missing measurement must not read as a passing one. Honest limit: the error budget is still marked provisional in the rules because it isn't yet calibrated per service, so I can defend the objectives and the gates, not an attainment figure. There is no on-call rotation — alerts route by service ownership, which for the observability backend, the release machinery and the import sidecar is me. **A sanitized copy of these rules is public**, thresholds, gates and reasoning intact, in [lgtm-stack-experiment](https://github.com/hjaraujof/lgtm-stack-experiment/blob/main/config/mimir/rules/demo/slo-burn.yaml) — so this is one of the few claims here you can check rather than take on trust.
...
Alongside it, in the same company's private codebase, I built the **blast-radius analysis service** behind the platform's IAM findings — a NestJS/TypeScript microservice that reimplements a meaningful slice of AWS IAM policy evaluation instead of calling the Policy Simulator: Allow/Deny precedence across six policy types, ~40 condition operators including CIDR containment, principal classification across accounts, roles, users, assumed-role, SAML and OIDC sessions with `Principal: "*"` expanded against the whole crawled ARN universe, and trust-graph reachability over `sts:AssumeRole`/`AssumeRoleWithSAML`/`AssumeRoleWithWebIdentity`. On top of that the multi-degree traversal the product is named for, propagating outward from a single ARN and merging findings per parent node. ~9,300 of its 11,200 non-test lines; its network-reachability engine is a colleague's work. More in [PROFILE.md](https://github.com/hjaraujof/hjaraujof/blob/main/PROFILE.md).
