...
**The private half** is the blast-radius analysis service, a NestJS/TypeScript microservice of 11,139 non-test lines, 11,040 of them mine — measured by `git blame` with move and copy detection against the trunk as it stood in March 2023, not estimated. The remaining 99 lines are a colleague's defensive fixes inside my files. That total includes the 1,905-line network-path analysis, of which 1,824 lines are mine. It reimplements a slice of AWS's own IAM policy evaluation rather than calling the Policy Simulator.
...
**Two more pieces of that platform.** In the internal resource-discovery service I own the AWS crawlers for S3, EMR, DynamoDB, CloudFormation, Elastic Beanstalk, SNS, SQS, CloudFront and network interfaces — 23 files at 76–100% of their surviving lines, roughly 4,250 lines. That is narrow ownership inside someone else's system: across the whole service I hold 6.7% of the lines and was its sixth contributor of 27. In the platform's GraphQL API I wrote the blast-radius surface — the assessment query at 99% of its lines, the mutation at 92%, and the client that drives the analysis service — and, unrelated to it, a 478-line snapshot-diff utility and its fixture suite, both entirely mine. Same caveat there: 5.3% of that repository overall, tenth contributor of 32. Narrow ownership inside larger systems is what most contribution actually looks like, and it is the part a commit count flatters.

These percentages come from `git blame` with move and copy detection, run against each repository's trunk as it stood in March 2023. They count surviving lines rather than effort.
