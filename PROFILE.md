...
**The connector platform:** per-document-type state machines and mappers with an explicit idempotency key threaded through the pipeline, a shared-contract system publishing Zod schema registries to a runtime config service, and a drift-guarded fake API whose route manifest is generated from the live controllers so consumer tests can't quietly go stale.
