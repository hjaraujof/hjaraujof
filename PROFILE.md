...
**Scale, and the two things I deliberately don't put numbers on.** Everything described here is carried by a single engineering team of under ten people. That is the honest frame for every ownership claim below: the scope per engineer is unusually wide, and "own" never means having a team underneath me. Two absences are deliberate. **There is no on-call rotation**: production alerts route by service ownership, which for the observability backend, the release machinery and the bulk-import sidecar means me, and incidents get worked the day they land rather than handed to a rota. And I quote **no request volumes and no uptime attainment**, because I would be reconstructing those rather than reading them. The service-level objectives I defined are in the observability section, with their thresholds and their calibration gap stated; the attainment against them is not something I can currently defend, so it isn't here.
...
**Service-level objectives, and the failure mode of an objective nobody can measure.** I wrote the fleet's SLO layer — windowed recording rules for availability and latency, then multi-window multi-burn-rate alerts on the Google SRE workbook pattern. A **99% availability** budget: a fast burn at **14.4×** the budget on the 1h *and* 5m windows pages, a slow burn at **6×** on the 6h *and* 30m windows opens a ticket, and both windows must breach so a brief spike cannot page. The latency objective is **95% of inbound requests under 250ms** over 30 minutes. Four decisions in there matter more than the thresholds do:

- **The numerator was wrong twice, and both times it measured instrumentation depth rather than failure.** Availability first ran on span *status* across every span kind, so one failing request marked every decorated method span it touched — a deeply instrumented service looked less available than a shallow one. The latency SLI had the same defect: one service emits roughly 500 internal spans per inbound request, so its measured fast-ratio read 0.92 while the real user-visible figure was 0.78, and a second service breached for 45 contiguous minutes on internal spans alone while serving 100% of inbound requests under 250ms. Adding instrumentation was moving the SLO with no user-visible change. Both are now restricted to server spans, and availability keys on HTTP status code rather than span status.
- **A 401 is not an availability breach, and it also cannot be invisible.** Under OpenTelemetry semantics a 4xx leaves server span status unset, so a sustained authentication failure across most of a service's traffic read as a healthy zero — invisible to every rule keyed on span status, and not logged either. It gets its own SLI, a sustained 401/403 ratio, deliberately excluded from the error budget because a service that authenticates a caller and says no is working correctly.
- **Sample-count gates derived rather than copied.** Each gate comes from the standard error at its own threshold band: 300 requests per 30 minutes for the latency ratio, where the error is about 1.3 points against a 5-point band, and 150 for the auth ratio, because that band is 10 points wide. Copying 300 across would have excluded the one service the auth alert exists to catch. An earlier rate-based gate was replaced after it kept flapping — the instability was sample count, not request rate.
- **An unmeasurable service alerts.** Restricting the denominator honestly meant the two highest-traffic services produced *no series* rather than a zero, because their framework's own tracing emits neither status code nor route. Dividing by all server spans instead would have given them an authoritative-looking 0% error rate — a false green on the busiest things in production. So silence there raises its own ticket, with the trade-off written into the rule: re-enabling the instrumentation that would supply the status code reintroduces a duplicate-span defect that was costing tens of thousands of errors a day.

The limit worth stating: the 1% error budget is still marked provisional in the rules, applied uniformly and not yet validated against a measured per-service baseline. The multi-window structure and the gates are the part I'd defend. Attainment against a budget I haven't finished calibrating is not.

Unusually for this document, you don't have to take my word for any of it: a sanitized copy of these rules is public in [lgtm-stack-experiment](https://github.com/hjaraujof/lgtm-stack-experiment/blob/main/config/mimir/rules/demo/slo-burn.yaml), with the thresholds, the sample gates and the reasoning comments intact.
